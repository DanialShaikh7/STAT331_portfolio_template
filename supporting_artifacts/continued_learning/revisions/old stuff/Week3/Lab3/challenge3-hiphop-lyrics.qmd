...
168 subjects we asked about their familiarity with African American English words. there is information about the individual such as their own demographic questions who are all not african american. they were asked the number of artists they like in each genre. we are making sense of this data.

```{r dataset-explore}
# you may want to use code to answer this question
hiphop <- read_csv(here("Week3","Lab3","hiphop.csv"))
dim(hiphop)
str(hiphop)
...
There are 10752 rows, within the data set background. It states there are 168 participants and 64 words. If you multiple 168 times 64 we get 10752, hence the number of rows.
...
So the missing values for the variables that were replaces were in the Move columns. you will notice that some variables in these columns are decimals and this is because instead of filling these missing values with 0 or random digits, they were filled with the average of the column so it wouldn't disrupt the average in the first place, if they had put 0 or 1 in there instead this would change the entire average of the data giving us a skewed data set.

```{r missing}
# you may want to use code to answer this question
mean(hiphop$asianMove)
mean(hiphop$blackMove)
```
...
As you can see we have a count of all the unique 64 words that studied in this dataset.
...
```{r}
# code chunk for Q7
hiphop_simplified <- hiphop |>
  mutate(ethnic_group = if_else(ethnic == "white", "white", "non-white"))
  hiphop_simplified
...
Collapsing ethnic or racial categories into broader categories, such as "white" and "non-white", can have several issues and limitations including 1. loss of detail of a data set, 2. Over generalization of the set, 3. stereotyping individuals in a white/ non white bucket, 3.inaccurate representation of individuals.
...
Some findings we can take away from this is the average age is 20 years old.
...
# select only the columns of interest and keep all columns
people <- distinct(hiphop, .keep_all = TRUE)

# create a bar plot to show the count of individuals by sex
ggplot(data = people, aes(x = sex)) + 
  geom_bar() + 
  ggtitle("Count of Individuals by Sex") + 
  xlab("Sex") + 
  ylab("Count")
...
There are 1772identities that fall under justin biebers criteria so it is very possible that one of them could be him however this is not guaranteed.
